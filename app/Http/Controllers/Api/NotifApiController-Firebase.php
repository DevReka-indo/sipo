...
class NotifApiController extends Controller
{
    ...
    {
        // Pastikan path dari .env benar
        $serviceAccount = base_path(env('FIREBASE_CREDENTIALS', 'storage/app/firebase/firebase-service-account.json'));
    ...
    public function sendToUser($id_user, $title, $body)
    {
        ...
        try {
            ...
            Log::error('❌ Gagal kirim notif', ['error' => $e->getMessage()]);
            return response()->json(
                [
                    'status' => false,
        ...
        // $tokenData = NotifTokenModel::where('id_user', $id_user)->pluck('token')->toArray();
    ...
    public function tesNotif(Request $request)
    {
        ...
        // 🔍 Kalau user_id dikirim, ambil semua token milik user
        if ($id_user && !$token) {
            $tokens = NotifTokenModel::where('id_user', $id_user)->pluck('token')->toArray();
        } elseif ($token) {
            $tokens = [$token];
            ...
            try {
                $message = CloudMessage::fromArray([
                    ...
                    'notification' => [
                        ...
                        'body' => 'Halo Master, ini pesan dari server Firebase!',
                    ],
                    'data' => ['customKey' => 'example'],
                ]);

                $response = $this->messaging->send($message);
                $success[] = ['token' => $t, 'response' => $response];
            } catch (\Throwable $e) {
                $failed[] = ['token' => $t, 'error' => $e->getMessage()];
            }
        }

        return response()->json([
            'status' => count($success) > 0,
            'message' => count($success) > 0 ? '✅ Notifikasi berhasil dikirim ke beberapa perangkat' : '❌ Gagal mengirim notifikasi ke semua perangkat',
            'success' => $success,
            'failed' => $failed,
        ]);
    }
