...
@push('scripts')
    <script>
        ...
        // Function untuk menampilkan konfirmasi delete dengan SweetAlert
        function showDeleteModal(undanganId, undanganTitle) {
            ...
            Swal.fire({
                ...
                // pakai class Bootstrap untuk warna tombol
                buttonsStyling: false,
                customClass: {
                    actions: 'swal2-actions-custom', // (opsional) beri jarak antar tombol
                    confirmButton: 'btn btn-danger px-4 py-2',
                    cancelButton: 'btn btn-secondary px-4 py-2 text-white' // <- abu-abu
                }
            }).then((result) => {
        ...
        function confirmDelete() {
            ...
            fetch(`/undangan/delete/${currentUndanganId}`, {
                    method: 'DELETE',
                    headers: {
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
                        'Content-Type': 'application/json',
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        Swal.fire({
                            title: 'Berhasil!',
                            text: data.message || 'Undangan berhasil dihapus',
                            icon: 'success',
                            confirmButtonText: 'OK',
                            confirmButtonColor: '#31CE36'
                        }).then(() => {
